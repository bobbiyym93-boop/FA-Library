...
const middleTick = computed(() => Number((maximum.value / 2).toFixed(maximum.value < 2 ? 1 : 0)))
...
<template>
  <section class="charts" aria-label="Dashboard charts">
    <article class="card product-chart"><h2>Product Distribution</h2><div class="product-chart-body"><div class="pie" :style="pieStyle"></div><ul class="legend"><li v-for="(item,index) in products" :key="item.name"><i :style="{background:colors[index % colors.length]}"></i><span :title="`${item.name} ${item.value} (${total ? Math.round(item.value / total * 100) : 0}%)`">{{ item.name }}&nbsp;&nbsp;{{ item.value }} ({{ total ? Math.round(item.value / total * 100) : 0 }}%)</span></li><li v-if="!products.length" class="muted">暂无数据</li></ul></div></article>
    <article class="card bar-chart"><h2>Cases by Project</h2><span class="axis-title-y">Case count</span><div class="plot dynamic-plot"><div class="y-axis-line"></div><div class="grid-line top"><span class="tick">{{ maximum }}</span></div><div class="grid-line middle"><span class="tick">{{ middleTick }}</span></div><div class="grid-line bottom"><span class="tick">0</span></div><div v-for="item in projects" :key="item.name" class="bar-column"><div class="bar" :style="{height:`${Math.max(4,item.value / maximum * 86)}px`}" :title="`${item.name}: ${item.value}`"></div><span>{{ item.name }}</span></div><span v-if="!projects.length" class="chart-empty">暂无数据</span></div><span class="axis-title-x">Project</span></article>
  </section>
</template>
